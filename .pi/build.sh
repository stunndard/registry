...
# docker buildx build --tag registry --file .pi/Dockerfile --platform linux/arm64 --cache-from registry:cache --cache-to registry:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar registry
...
sudo bash -c "echo registry > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/registry.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/registry.img
